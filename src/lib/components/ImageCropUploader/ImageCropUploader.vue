<template>
  <a-modal
    wrapClassName="vue-image-crop-upload"
  ...
  >
    ...
    <template #footer>
      <template v-if="[1].includes(step)">
        <a-button @click="off">{{ lang.btn.off }}</a-button>
      </template>
      <template v-if="[2].includes(step)">
        <a-button @click="setStep(1)">{{ lang.btn.back }}</a-button>
        <a-button type="primary" @click="prepareUpload">{{ lang.btn.save }}</a-button>
      </template>
      <template v-if="[3].includes(step)">
        <a-button @click="setStep(2)">{{ lang.btn.back }}</a-button>
        <a-button @click="off">{{ lang.btn.close }}</a-button>
      </template>
    </template>
  </a-modal>
</template>
...
export default {
  ...
  methods: {
    ...
    prepareUpload() {
      let { url, createImgUrl, field, uuid } = this
      this.$emit('crop-success', createImgUrl, field, uuid)
    ...
    upload() {
      let that = this,
        { lang, imgFormat, mime, url, params, headers, field, uuid, createImgUrl, withCredentials, method } = this,
        fmData = new FormData()
      ...
      new Promise(function (resolve, reject) {
        ...
        function (resData) {
          that.loading = 2
          that.$emit('crop-upload-success', resData, field, uuid)
          that.visible = false
        },
        // 上传失败
        function (sts) {
          ...
          that.$emit('crop-upload-fail', sts, field, uuid)
        }
      )
    },
  },
...
.vue-image-crop-upload
  ...
  background-color: rgba(22, 119, 255, 0.45);
}
...
.vue-image-crop-upload .vicp-wrap .vicp-step3 .vicp-upload .vicp-progress-wrap .vicp-progress {
  ...
  border-radius: 3px;
  background-color: #1677ff;
  -webkit-box-shadow: 0 2px 6px 0 rgba(22, 119, 255, 0.3);
  box-shadow: 0 2px 6px 0 rgba(22, 119, 255, 0.3);
...
.vue-image-crop-upload .vicp-wrap .vicp-step3 .vicp-upload .vicp-progress-wrap .vicp-progress::after {
  ...
  -webkit-box-shadow: 0 1px 4px 0 rgba(22, 119, 255, 0.7);
  box-shadow: 0 1px 4px 0 rgba(22, 119, 255, 0.7);
  border-radius: 100%;
  background-color: #1677ff;
}
