<template>
  <div class="deadline">
    <svg preserveAspectRatio="none" id="line" viewBox="0 0 581 158" enable-background="new 0 0 581 158">
      <g id="fire">
        ...
        <g>
          ...
          <g id="group-fire" clip-path="url(#mask-fire_1_)">
            <path
              id="red-flame"
              :opacity="percent >= 80 && percent < 100 ? '1' : '0'"
              fill="#B71342"
            ...
            <path
              id="yellow-flame"
              :opacity="percent >= 80 && percent < 100 ? '0.71' : '0'"
              fill="#F7B523"
            ...
            <path
              id="white-flame"
              :opacity="percent >= 80 && percent < 100 ? '0.81' : '0'"
              fill="#FFFFFF"
      ...
      <g id="death-group" :style="{ transform: `translateX(${translateX}px)` }">
        <path
          id="death"
          fill="#BE002A"
      ...
      <g id="designer-arm-grop" :style="{ animationDuration }">
        <path
          id="designer-arm"
          fill="#FEFFFE"
    ...
    <div class="deadline-days" v-if="showLoadingText">
      <div class="mask-red" :style="{ width: 140 * percent / 100 + 'px' }">
      <div class="inner">
        {{ LoadingText }} <span class="day">{{ Number(percent.toFixed(2)) }}</span> <span class="days">%</span>
      </div>
    </div>
...
html[color-scheme="light"] {
  #designer-body, #designer-head {
    fill: #000000;
  }
  #mask-fire-black {
    fill: #ffffff;
  }
  #progress-trail, #designer-arm-grop {
    path {
      fill: #000000;
    }
  }
...
#red-flame {
  // opacity: 0;
  animation: red-flame 120ms ease infinite;
  transform-origin: center bottom;
}

#yellow-flame {
  // opacity: 0;
  animation: yellow-flame 120ms ease infinite;
  transform-origin: center bottom;
}

#white-flame {
  // opacity: 0;
  animation: red-flame 100ms ease infinite;
  transform-origin: center bottom;
}
